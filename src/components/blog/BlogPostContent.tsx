    ...
    while (currentIndex < lines.length) {
      ...
      if (line.includes('|') && line.includes('Mistake') && line.includes('Fix')) {
        ...
        if (tableData.length > 0) {
          elements.push(<div key={currentIndex} className="my-6">
              <div className="overflow-hidden rounded-xl shadow-lg border-0">
                <Table>
                  ...
                  <TableBody>
                    {tableData.map((row, idx) => <TableRow key={idx} className={idx % 2 === 0 ? "bg-white hover:bg-blue-50" : "bg-blue-50/50 hover:bg-blue-100"}>
      ...
      if (line.startsWith('# ')) {
        elements.push(<div key={currentIndex} className="text-center my-6">
        ...
        elements.push(<div key={currentIndex} className="my-4">
            <div className="flex items-center gap-3 mb-3">
              {isNumbered && <span className="bg-gradient-to-r from-blue-500 to-purple-500 text-white w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold shadow-lg">
                  {title.match(/^\d+/)?.[0]}
                </span>}
        ...
        if (altMatch && urlMatch) {
          elements.push(<div key={currentIndex} className="my-5">
        ...
        if (line.includes('Get Started:') || line.includes('Why It Shines:') || line.includes('Pro Tip:') || line.includes('Limitation:')) {
          ...
          elements.push(<div key={currentIndex} className={`my-3 ${bgColor} ${borderColor} border-l-4 rounded-r-lg p-3`}>
          ...
          elements.push(<p key={currentIndex} className="mb-2 leading-relaxed text-gray-700 text-base">
              {cleanText}
            </p>);
        }
      }
